...
circ_buff_code circ_buff_init(CIRC_BUFF* circ_buff_pointer, int16_t size)
{
    ...
    circ_buff_pointer->buff=(uint8_t*)malloc(size);    //access the buff pointer and allocate memory on the heap
    
    if(circ_buff_pointer->buff==NULL)
    ...
    /*Initialise the head and tail positions to 0*/
    circ_buff_pointer->head=0;                        
    circ_buff_pointer->tail=0;   
...
circ_buff_code if_circ_buff_full(CIRC_BUFF* circ_buff_pointer)
{
    ...
    uint16_t current_buff_size=circ_buff_pointer->size_occupied, total_buff_size= circ_buff_pointer->total_size;
...
circ_buff_code if_circ_buff_empty(CIRC_BUFF* circ_buff_pointer)
{
    ...
    uint16_t current_buff_size=circ_buff_pointer->size_occupied;  //read current buffer size
...
circ_buff_code circ_buff_write(CIRC_BUFF* circ_buff_pointer, uint8_t data)
{
    ...
    uint16_t total_buff_size= circ_buff_pointer->total_size;
    

    *(circ_buff_pointer->buff + (circ_buff_pointer->tail)%total_buff_size)=data;  //find the tail and write to it

    circ_buff_pointer->tail=(circ_buff_pointer->tail+1)%total_buff_size;           //update tail circularly
...
circ_buff_code circ_buff_read(CIRC_BUFF* circ_buff_pointer, uint8_t* data)
{
    ...
    *data= *(circ_buff_pointer->buff + circ_buff_pointer->head);  //assign the byte located at head to data and complete the read 

    circ_buff_pointer->head=(circ_buff_pointer->head+1)%total_buff_size;           //update head circularly
