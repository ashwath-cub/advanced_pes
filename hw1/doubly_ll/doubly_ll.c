...
dll_code dll_add_node(dll_node_ptr* head, uint32_t position, uint32_t data)
{
    ...
    if(*head==NULL)
	 return DLL_NULL_PTR;                                               //check if the pointer is NULL
 ...
 *               
 *              
 * Usage:        Pass a pointer to the pointer containing the head of the dll.
 * 
 * Returns:      Error codes:
 ...
 *               
 * Working:      Takes a ptr to the head ptr and traverses the dll to reach 
 *               position-1, grabs node at position and frees its memory. Links 
 *               node before and after position safely.
 * 
 * Returns:      Error codes:
 *               DLL_NULL_PTR: The pointer passed to the function is a
 *               NULL and is thus invalid. The function halts execution and 
 ...
 * ----------------------------------------------------------------------------
 */
dll_code dll_remove_node(dll_node_ptr* head, uint32_t position)
{
    //basic pointer check; error handling	
    if(head==NULL)
	 return DLL_NULL_PTR;
    ...
    if(position==0)
    {
         if((*head)->next_ptr!=NULL)                  //if not the size is 1
	      *head=(*head)->next_ptr;
	 (*head)->prev_ptr=NULL;
	 free(tmp_head);
	 return DLL_SUCCESS;
 ...
 *      
 * Usage:        Pass a pointer to the pointer of the head of the dll, a pointer
 ...
 *               input pointer of the dll with head= *head.
 *               
